...
\begin{frame}{O que significa \textit{logit}?}\justifying
    O termo \textbf{logit} vem da estatística e originalmente está
    relacionado ao \textbf{logaritmo das odds}.

    \vspace{0.3cm}

    As \textbf{odds} representam a razão entre a probabilidade de um
    evento ocorrer e a probabilidade de ele não ocorrer:

    \[
        \operatorname{odds}(p)=\frac{p}{1-p}
    \]

    Portanto, o \textit{logit} é definido como:
    ...
    No caso binário, essa transformação é a \textbf{inversa da
    função sigmoide}:
...
\begin{frame}
    \begin{figure}
        \centering
        \includegraphics[width=0.5\linewidth]{pic/neural_network.png}
...
\begin{frame}{Derivando o \textit{logit}}\justifying
    Partimos da função de ativação sigmoide:
...
\begin{frame}{\textit{Logits} em modelos multiclasses}\justifying

    Ao passar de duas classes para várias classes, utilizamos a
    função \textbf{softmax}:
    ...
    Os valores $z_i$ produzidos pelo modelo são chamados de
    \textbf{logits}.

    \vspace{0.3cm}

    Diferentemente do caso binário, cada $z_i$ \textbf{não é,
    individualmente, o logaritmo das odds}:
    ...
    O que podemos obter é a diferença entre dois logits:

    \[
        \boxed{
            z_i-z_j=
            \ln\left(\frac{p_i}{p_j}\right)
        }
    \]

    \vspace{0.2cm}

    Nas LLMs, o termo \textit{logit} é utilizado para esses
    \textbf{scores brutos antes do softmax}.
...
\begin{frame}{Amostragem por Temperatura — Exemplo}
    \justifying

    \vspace{0.2cm}

    A função \textbf{softmax com temperatura} é definida por:

    \[
        \operatorname{softmax}_i\left(\frac{\mathbf{z}}{T}\right)
        =
        \frac{e^{z_i/T}}
        {\displaystyle\sum_{j=1}^{K} e^{z_j/T}}
    \]

    \vspace{0.15cm}

    Considere os valores obtidos por uma rede feedforward:
    \[
        z_1=12,\qquad z_2=13,\qquad z_3=4.
    \]
    ...
    \begin{tabular}{c|ccc}
        \hline
        \textbf{Temperatura} & $P(z_1)$ & $P(z_2)$ & $P(z_3)$ \\
        \hline
        $T=0.1$ &
        $4.54\times10^{-5}$ &
        $0.9999546$ &
